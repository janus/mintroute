...
fn parse_place_holders(route: &str) -> Vec<RouteParser> {
    ...
    for capture in re.captures_iter(route) {
        ...
        let rt_parser = RouteParser { route_datas: vec![static_path, dymanic_variable] };
...
mod tests {
    ...
    #[test]
    fn with_options() {
        let mvec: Vec<Vec<RouteParser>> =
            parse("/fixedRoutePart[/{varName}/moreFixed/{varName2:[0-9]+}]");
